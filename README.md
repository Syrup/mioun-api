...
const mioun = require('mioun-api');
const api = new mioun();
const git = api.github('miouns').then(body => {
  console.log(body.login); // => Miouns
})
```
...
(async () => {
  ...
  console.log(anime); // => {"url":"http://imgur.com/7gZvDdl.jpg"}
  console.log(anime.url) // => http://imgur.com/
})
```
